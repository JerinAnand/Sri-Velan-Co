...
export const EquipmentTab: React.FC<EquipmentTabProps> = ({ content, updateSection }) => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      await updateSection('equipment', { fleetIntro, categories });
      setSaveSuccess(true);
  ...
  const createNewCategory = (): EquipmentCategoryItem => ({
    id: `eq_${Date.now()}`,
    ...
    <form onSubmit={handleSave} className="space-y-6">
      <div className="flex items-center justify-between border-b border-white/10 pb-4">
        <div>
          ...
          <p className="text-xs text-neutral-400">Manage equipment fleet intro, machinery categories, quantities, images, and technical specifications.</p>
        </div>
        <button
          type="submit"
          disabled={saving}
      ...
      {saveSuccess && (
        <div className="p-3 bg-emerald-500/10 border border-emerald-500/30 rounded-xl text-xs text-emerald-400 flex items-center gap-2 font-mono">
          ...
          Equipment fleet updated successfully in Firestore real-time!
        </div>
      )}
