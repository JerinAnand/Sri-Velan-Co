...
export const ServicesTab: React.FC<ServicesTabProps> = ({ content, updateSection }) => {
  ...
  return (
    <form onSubmit={handleSave} className="space-y-6">
      <div className="flex items-center justify-between border-b border-white/10 pb-4">
        <div>
          <h2 className="text-lg font-display font-bold text-white">Services Section</h2>
          <p className="text-xs text-neutral-400">Manage services cards, titles, descriptions, highlights, and images.</p>
        </div>
        <button
          type="submit"
          disabled={saving}
      ...
      {saveSuccess && (
        <div className="p-3 bg-emerald-500/10 border border-emerald-500/30 rounded-xl text-xs text-emerald-400 flex items-center gap-2 font-mono">
          ...
          Services updated successfully in Firestore real-time!
        </div>
      )}
