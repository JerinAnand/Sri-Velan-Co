...
export const CapabilityStatement: React.FC = () => {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      // Configure A4-optimized settings
      // Using explicit hex values instead of oklch to prevent html2canvas parsing errors
      const opt = {
        margin: [8, 12, 8, 12] as [number, number, number, number], // Minimal margin in mm to fit everything onto one page perfectly
        filename: 'Sri-Velan-Co-Capability-Statement.pdf',
      ...
      // Informative alert to user and automatic fallback to printing
      alert('An error occurred during automatic PDF generation. Opening system print dialog as a secure fallback.');
      window.print();
    } finally {
