...
export const handler = async (event: any, context: any) => {
  ...
  try {
    ...
    console.warn("[Sri Velan Netlify API] Real Gemini API call failed. Deploying fallback local engineering planner: ", err.message || err);
    try {
      ...
      console.error("Severe: Fallback generator failed in Netlify handler:", fallbackErr);
      return {
        statusCode: 500,
        headers,
