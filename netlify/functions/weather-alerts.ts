...
export const handler = async (event: any, context: any) => {
  ...
  try {
    ...
    if (apiKey) {
      try {
        ...
        try {
          ...
          console.warn("[Netlify Weather Alerts API] Gemini search grounding failed, retrying with telemetry-grounded prompt without search tool:", searchErr.message);
        ...
        console.warn("[Netlify Weather Alerts API] Gemini call failed completely, using backup text:", aiErr.message);
    ...
    console.error("Critical netlify weather alert API error:", globalErr);
    return {
      statusCode: 500,
      headers,
      body: JSON.stringify({ error: globalErr.message || "Internal server error fetching alerts." })
    };
  }
};
